...
while(<IN>) {
  ...
  if ($depth > 9) {
    ...
    if ($mis ne "N" and $misRate > 0.07) {
      ...
      $ID2bases_tum{$curRow[0] . "\t" . $curRow[1]} = $curRow[2] . "\t" . $mis . "\t" . $curRow[3] . "\t" . $A . "\t" . $C . "\t" . $G . "\t" . $T . "\t"  . $misRate . "\t" . $sratio;
    }
  }
}
close(IN);
...
while(<IN>) {
  ...
  if (exists $ID2bases_tum{$curRow[0] . "\t" . $curRow[1]} and $depth > 9) {

    @bases_tum = split("\t", $ID2bases_tum{$curRow[0] . "\t" . $curRow[1]});
    $mis = $bases_tum[1];
    $mis =~ tr/a-z/A-Z/;
    ...
    print $curRow[0] . "\t" . $curRow[1] . "\t" . $ID2bases_tum{$curRow[0] . "\t" . $curRow[1]} . "\t" . $curRow[3] . "\t" . $A . "\t" . $C . "\t" . $G . "\t" . $T . "\t"  . $misRate . "\t" . $sratio . "\n";
  }
}
